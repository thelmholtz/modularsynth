...
export class Amp {
  constructor(audioInstance, outputs, options = NODE_DEFAULTS) {
    ...
    this.amp.gain.linearRampToValueAtTime(
      UNITY,
      this.audioInstance.currentTime + this.env.attack
    )
    this.amp.gain.linearRampToValueAtTime(
      this.env.sustain * UNITY,
      this.audioInstance.currentTime + this.env.decay
    )
  ...
  stop() {
    this.amp.gain.linearRampToValueAtTime(
      SILENCE,
      this.audioInstance.currentTime + this.env.release
    )
  }
